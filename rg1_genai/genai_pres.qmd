...
trend-cyclical and seasonal parts from the series progressively.

For an input series $X_t$ with length $L$, the decomposition layer returns 
$X_{trend}$ and $X_{seasonal}$, both of length $L$.  


## Autoformer - Attention Mechanism

Autoformer uses **autocorrelation within the self-attention** mechanism, extracting
frequency-based dependencies from queries and keys (instead of the standard 
dot-product).  
...
(similar to Informer's ProbSparse attention).
